...
class Random3dCanvas[S <: Seq[Cell], T <: Seq[S]](width: Int, height: Int, index: Int, layers: => Layers[S, T]) extends Array3dCanvas[S, T](Seq.empty[S].asInstanceOf[T], index, layers, (for (x <- 0 until width; y<-0 until height) yield (x, y)).toSet) {


  override def stage(): Canvas[S, T] = {
    new Array3dCanvas[S, T](canvas.asInstanceOf[T], index, layers, changedCells)
  }
