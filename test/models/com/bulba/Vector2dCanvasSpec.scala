...
class Vector2dCanvasSpec extends FlatSpec with ShouldMatchers {
  ...
  "2d array canvas" should "populate set of changed cells" in {
      val canvas = new Vector2dCanvas[Seq[Cell], Seq[Seq[Cell]]](Seq(Seq(LiveCell, LiveCell), Seq(DeadCell, LiveCell), Seq(DeadCell, DeadCell)), Set((0,1), (0, 0), (1, 0), (1,1)))
  ...
  "2d array canvas" should "not stage cells that have not changed since previous iteration" in {
    val canvas = new Vector2dCanvas[Seq[Cell], Seq[Seq[Cell]]](Seq(Seq(LiveCell, LiveCell),
